package net.coderbot.iris.renderdoc;
...
public class RenderDoc {
	...
	static {
		...
		if (os == Util.OS.WINDOWS || os == Util.OS.LINUX) {
			try {
				...
				if (initResult != 1) {
					Iris.logger.error("Could not connect to RenderDoc API, return code: {}", initResult);
				} else {
					...
					Iris.logger.info("Connected to RenderDoc API v" + major.getValue() + "." + minor.getValue() + "." + patch.getValue());
				}
	...
	public static void load() {
		Iris.logger.warn("Attempting to load Renderdoc");
	}
